...
/** @addtogroup N32H47X_StdPeriph_Template
 * @{
 */
...
/******************************************************************************/
/*                 N32H47X Peripherals Interrupt Handlers                   */
...
/**
*\*\name    GTIM1_IRQHandler.
*\*\fun     This function handles GTIM1 global interrupt request.
*\*\param   none
*\*\return  none 
**/
...
{
    RCC_ClocksType clks;
    if(TIM_GetIntStatus(GTIM1, TIM_INT_CC3) != RESET)
    {
        if (CaptureNumber == 0)
        {
            /* Get the Input Capture value */
            IC1ReadValue1 = TIM_GetCap3(GTIM1);
        }
        else if (CaptureNumber == 2)
        {
            /* Get the Input Capture value */
            IC1ReadValue2 = TIM_GetCap3(GTIM1);
            ...
            LsiFreq = (uint32_t)clks.Pclk1Freq / (2*Capture);
            LsiFreq *= 32;

        }
        CaptureNumber++;
        /* Clear GTIM1 Capture compare interrupt pending bit */
        TIM_ClrIntPendingBit(GTIM1, TIM_INT_CC3);
    }
}
