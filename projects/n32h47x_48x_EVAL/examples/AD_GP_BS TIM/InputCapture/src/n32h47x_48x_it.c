...
__IO uint32_t TIMxFreq        = 0;
__IO uint32_t TIMxEnterIrqCnt = 0;
...
void TIMx_IRQHandler(void)
{
    if (TIM_GetIntStatus(TIMx, TIM_INT_CC2) == SET)
    {
        ...
        if (CaptureNumber == 0)
        {
            ...
            IC3ReadValue1 = TIM_GetCap2(TIMx);
            CaptureNumber = 1;
        }
        else if (CaptureNumber == 1)
        {
            /* Get the Input Capture value */
            IC3ReadValue2 = TIM_GetCap2(TIMx);
            ...
            TIMxFreq      = (uint32_t)(SystemCoreClock/2) / Capture;
            CaptureNumber = 0;
        }
    }
}
