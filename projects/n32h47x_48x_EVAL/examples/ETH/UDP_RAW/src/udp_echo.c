...
static struct udp_pcb *udp_raw_pcb = NULL;
...
/**
*\*\name    udp_recv_callback.
*\*\fun     Recv callback function.
*\*\param   arg
*\*\         - User supplied argument
*\*\param   upcb
*\*\         - The udp_pcb which received data
*\*\param   p
...
void udp_echo_init(void)
{
    ...
    /* Creates a new UDP protocol control block */
    udp_raw_pcb = udp_new_ip_type(IPADDR_TYPE_ANY);

    /* Bind local IP address and port */
    err = udp_bind(udp_raw_pcb, IP_ANY_TYPE, LOCAL_PORT);
    if (err == ERR_OK)
    {
        printf("Bind local IP address and port successful!!\n");
        printf("Local port: %d\n", LOCAL_PORT);
        /* Registers a callback function to receive data */
        udp_recv(udp_raw_pcb, udp_recv_callback, NULL);
    }
    else
    {
