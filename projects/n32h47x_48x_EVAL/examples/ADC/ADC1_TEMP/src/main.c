...
CMD_RETURN_CR  (*Get_NVR)(uint32_t addr, uint32_t* data, uint32_t len) = (CMD_RETURN_CR (*)(uint32_t ,uint32_t*, uint32_t))0x1FFF02A5;
...
{ 
    ...
    Temperate=(VTS_value-(tempValue*1000))/(AVG_SLOPE*1000)+T_value/100-1.25f;
...
void ADC_Initial(void)
{
    ...
    ADC_ConfigRegularChannel(ADC1, ADC1_Channel_16_Temperture_Sensor, 1, ADC_SAMP_TIME_CYCLES_601_5);
    /* Enable Temper Sensor */
...
int main(void)
{
    ...
    while(ADC_GetFlagStatus(ADC1,ADC_FLAG_EOC_ANY) == RESET);
...
void RCC_Configuration(void)
{
    ...
    /* RCC_ADCHCLK_DIV10*/
    ADC_ConfigClk(ADC_CTRL3_CKMOD_AHB,RCC_ADCHCLK_DIV10);
