...
__IO uint16_t ADCConvertedValue[DMA_SIZE];
...
void ADC_Initial(void)
{
    ...
    /* Config ADC1 FIFO waterlevel = 6 */
    ADC_ConfigFIFOWaterLevel(ADC1,6);
    /* Enable ADC1 FIFO */
    ADC_EnableFIFO(ADC1,ENABLE);
    /* Enable ADC1 */
...
int main(void)
{
    ...
    while (1)
    {
        while(!ADC_GetFlagStatus(ADC1,ADC_FLAG_EOC_ANY))
        ADC_ClearFlag(ADC1,ADC_FLAG_EOC_ANY) ;
        ADC_EnableSoftwareStartConv(ADC1, ENABLE);
...
void DMA_Config(void)
{
    ...
    DMA_InitStructure.BufSize        = DMA_SIZE;
