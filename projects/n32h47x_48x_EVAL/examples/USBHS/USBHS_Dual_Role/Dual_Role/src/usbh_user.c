...
USBH_User_cb_TypeDef USER_Host_MSC_cb = 
{
    USBH_USER_Init,
    USBH_USER_DeInit,
...
void USBH_USER_Device_DescAvailable(void *DeviceDesc)
{
  ...
  sprintf((char *)temp, "VID : %04luh\n", (uint32_t) (*hs).idVendor);
  log_info((void *)temp);

  sprintf((char *)temp, "PID : %04luh\n", (uint32_t) (*hs).idProduct);
  log_info((void *)temp);
}
...
USBH_USER_Status USBH_USER_UserInput(void)
{
    ...
    if (GPIO_ReadInputDataBit(KEY_BUTTON_GPIO_PORT, KEY_BUTTON_PIN) == RESET)
    {
