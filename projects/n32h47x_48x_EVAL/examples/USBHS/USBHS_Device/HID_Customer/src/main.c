...
int main(void)
{
    ...
    USBD_Init(&USB_dev, &USBD_desc, &USBD_CUSTOMHID_cb, &USER_cb);

    while (1)
    {
        if (PrevXferDone && (USB_dev.dev.device_status == USB_CONFIGURED))
        {
            ...
            USBD_CUSTOM_HID_SendReport(&USB_dev, Send_Buffer, 2);
            PrevXferDone = 0;
        }
    }
}
