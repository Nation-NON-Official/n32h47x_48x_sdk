...
uint8_t Rxbuffer[CDC_DATA_MAX_PACKET_SIZE];
...
extern void USB_BSP_mDelay (const uint32_t msec);
/**
*\*\name    main.
*\*\fun     Main program.
*\*\param   none.
...
int main(void)
{
    ...
    USBD_Init(&USB_dev, &USBD_desc, &USBD_CDC_cb, &USER_cb);
    ...
    while (1)
    {
        ...
        {
            /* receive character */
            VCP_ReceiveData(&USB_dev, Rxbuffer, receive_count);
            /* send myTxBuffer */
            VCP_SendData(&USB_dev, Rxbuffer, receive_count);
