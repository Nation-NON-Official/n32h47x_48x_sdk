...
extern uint32_t RxFlag;
extern uint32_t TxFlag;
extern uint32_t CompFlag;
...
{
    if(FDCAN_GetIntFlag(NODE2,FDCAN_FLAG_RX_FIFO1_WATERMARK) == SET)
    {
        RxFlag = 1;
        FDCAN_ClearFlag(NODE2,FDCAN_FLAG_RX_FIFO1_WATERMARK);
    }
}
