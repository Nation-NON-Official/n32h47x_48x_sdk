...
/**
*\*\name    LptimEncodeConfig.
*\*\fun     Initialize configuration LPTIM encode mode.
*\*\param   none
*\*\return  none
**/
void LptimEncodeConfig(void)
{
    /* Reset LPTIM1 */
    LPTIM_DeInit(LPTIM1);
    ...
    /* Enables the encoder mode of LPTIM1 */
...
/**
*\*\name    LptimEncodeWaveOutput.
*\*\fun     Generate encode waves output.
*\*\param   Count
*\*\          - The number of waves
*\*\return  none
**/
void LptimEncodeWaveOutput(uint16_t Count)
{
    while(Count--)
    {
...
int main(void)
{
    ...
    LptimEncodeWaveOutput(20U);

    /* Get actual counter value */
    encCNT = LPTIM_GetCounterValue(LPTIM1);
    if (encCNT != 40U)
    {
        log_info("LPTIM_ENC demo failed!!!\n");
    }
    else
    {
        log_info("LPTIM_ENC demo success!!!\n");
    }
