...
3、使用说明
    ...
    使用方法：
        在KEIL下编译后烧录到评估板。
        开启打印输出，可以看到串口输出“CORDIC_Fixed_Sin_DMA demo go...”；若计算结果与参考值的差大于最大误差值，则串口输出“Error”；若计算结果与参考值的差小于等于最大误差值，则串口输出“CORDIC demo success!!!”，指示计算结果符合预期。
...
3. Instructions for use
    ...
    Cordic configuration:
     ...
     Instructions:
        ...
        Turn on the printout, you can see the serial output "CORDIC_Fixed_Sin_DMA demo go..."; If the difference between the calculated result and the reference value is greater than the maximum error value, the serial port outputs "Error"; if the difference between the calculated result and the reference value is less than or equal to the maximum error value, the serial port outputs "CORDIC demo success!!!", indicating that the calculated result meets the expectation.
