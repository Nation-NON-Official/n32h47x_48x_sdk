...
/**
*\*\name    SPI_DMA_PageWrite.
*\*\fun     Writes block of data to the FLASH by DMA. 
*\*\param   WriteAddr FLASH's internal address to write to.
*\*\return  none
**/
void SPI_DMA_PageWrite(uint32_t WriteAddr)
{
    ...
    /* Wait for sFLASH_SPI transfer complete */
    while(SPI_I2S_GetStatus(sFLASH_SPI, SPI_I2S_BUSY_FLAG));
                        
     /* Disnable sFLASH_SPI Tx request */
    ...
    /*!< Set the sFLASH_SPI Doubleline FullDuplex*/
    SPI_InitStructure.DataDirection = SPI_DIR_DOUBLELINE_FULLDUPLEX;
    SPI_Init(sFLASH_SPI, &SPI_InitStructure);
...
/**
*\*\name    SPI_DMA_BufferRead.
*\*\fun     Reads a block of data from the FLASH by DMA.
*\*\param   ReadAddr FLASH's internal address to read from.
*\*\return  none
**/
